...
$(document).on('click', '.limpar-tabela', function(e) {
 ...
 $(document).on('click', '.cadastro-proximo2', function(e) {
	...
	if (!table.length) {
		...
		var string = window.location.href.split('/')[3];
		$.post(string+'/cadastro/turmas/'+'?ajax=1', f2.serialize(), function(data) {
			if (data) {
				$('.form1, .form2').hide();
				$('.step').removeClass("active-step");
...
$(document).on('click', '.cadastro-anterior3', function(e) {
 ...
 $(document).on('click', '.cadastro-finalizar', function(e) {
	...
	var f = $('form');
		var string = window.location.href.split('/')[3];
		$.post(string+'/cadastro/finalizar/'+'?ajax=1', f.serialize(), function(data) {
			if (data == 1) {
				$('.form1, .form2, .form3').hide();
...
$(document).on('click', '.load-avaliadores', function(e) {
	...
	if (re.html()) {
		re.html('');
			...
			var string = window.location.href.split('/')[3];
			$.post(string+'/acompanhamento/load_avaliadores/', {id:id}, function(data) {
			re.html(data);
		});
	}
});
