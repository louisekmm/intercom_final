...
?>
	<div class='row'>
		<ol class="breadcrumb">
			...
			?>
			<li ><a href="<?=$proj?>/<?=$this->meta['controller']?>/add/0/<?=$this->args?>" class='btn btn-primary btn-xs ' data-toggle="tooltip" data-placement="top" title="<?=$this->lng['adicionar_cadastro']?>"><i class='glyphicon glyphicon-plus'></i></a></li>
			
			<?php
			if ($this->meta['import']) {
			?>
			<li ><a href="<?=$proj?>/<?=$this->meta['controller']?>/import/<?=$this->args?>" class='btn btn-primary btn-xs ' data-toggle="tooltip" data-placement="top" title="<?=$this->lng['importacao_list']?>"><i class='glyphicon glyphicon-film'></i></a></li>
		...
		?>
			<a href="<?=(URL.ADMIN_FOLDER.'/'.$proj.'/'.$this->meta['controller'].'/order/'.$this->args)?>" class='btn btn-warning disabled btn-order'><?=$this->lng['salvar_list']?></a>
		<?php
			}
		?>
			...
			<tr>
				...
				<?php
					if (isset($this->extraActions)) {
						...
						?>
						<a href='<?=$proj?>/<?=str_replace('%id', $result['id'], $obj['href'])?><?=$result['id']?>/' class='btn <?=$obj['btn-class']?> btn-xs <?=$obj['class']?>' data-toggle="tooltip" title="<?=$obj['name']?>" target="<?=(isset($obj['target']) ? $obj['target'] : '')?>">
				...
				?>
					...
					?>
					<a href='<?=$proj?>/<?=$this->meta['controller']?>/add/<?=$result['id']?>/<?=$this->args?>' class='btn btn-primary btn-xs' data-toggle="tooltip" title="<?=($this->meta['permissions']['add'] ? $this->lng['editar_cadastrador_completar_widget'] : $this->lng['ver_list'])?>">
						<?php
					...
					?>
						<a href='<?=$proj?>/<?=$this->meta['controller']?>/del/<?=$result['id']?>/' class='btn btn-danger btn-xs remove-item' data-toggle="tooltip" title="<?=$this->lng['remover_list']?>"><i class='glyphicon glyphicon-remove'></i></a>
					<?php
					}
					?>
				...
				<?php
						...
						if ($column['reference']) {
							$value = '<a href="'.URL.ADMIN_FOLDER.'/'.$proj.'/'.$column['reference'].'/list/1/'.$result['id'].'/" class="btn btn-default"><i class="glyphicon glyphicon-th-list"></i></a>';
						} else {
							//print_r($column);
							...
							if (DefaultController::isCheckBox($column['options'])) {
								$palavra = $this->lng['sim-acompanhamento'];
								$palavra1 = $this->lng['nao-acompanhamento'];
								$value = ($value ? $palavra : $palavra1);
							}
	  ...
	  <ul class="pagination">
		<?php
			if ($this->page == 1) {
				...
				echo '<li><a href="'.$proj.'/'.$this->meta['controller'].'/list/'.($this->page-1).'/">';?>
				<?=$this->lng['anterior_cadastro']?>
				<?php echo '</a></li>';
			}
		?>
	...
	for($i=1; $i <= $this->totalPages; $i++) {
		...
		echo '<li '.$active.'><a href="'.$proj.'/'.$this->meta['controller'].'/list/'.$i.'/">'.$i.'</a></li>';
	}
	?>	
		<?php
