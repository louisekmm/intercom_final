<div id="page-wrapper">
	<div class='row'>
		...
		<form method='post' action='<?=$proj?>/entidade/salva_avaliacao/'>
...
<br>
		...
		?>
			<tr>
					...
					<td>
					<!--ESTRELAS:<input type="hidden" <?=($this->semana['expirado'] ? 'data-readonly' : '')?> class="rating1" name='nota[<?=$result['idPessoa']?>]' id='nota[<?=$result['idPessoa']?>]' value='<?=$result['nota']?>' data-filled="glyphicon glyphicon-star gi-2x" data-empty="glyphicon glyphicon-star-empty gi-2x">-->

					<select name="nota[<?=$result['idPessoa']?>]" id="nota[<?=$result['idPessoa']?>]"  class='required form-control' <?=($this->semana['expirado'] ? 'disabled' : '')?>>
							...
							<?php
								...
								foreach($this->results1 as $result1) {
									...
									if($valor == $nota){
									?>
									<option selected value="<?=$result1['valor']?>">
										<?=$result1['descricao']?>
									</option>
									<?php
									}else{
									?>
										<option value="<?=$result1['valor']?>">
										<?=$result1['descricao']?>
									</option>
									<?php
									}
