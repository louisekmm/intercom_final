	...
	<div class='col-md-9 '>
		...
		<div class='text-center'>
			...
			<ul class='pagination'>
				...
				?>
				<li class='<?=($x == $this->indice ? 'active' : '')?>'><a href="<?=$proj?>/formulario/responder/<?=$this->dados['slug']?>/perguntas/<?=$x?>/"><?=$x?></a></li>
				<?php
				}
				?>
			</ul>
		...
		<form class='form' method='post' action='<?=$proj?>/formulario/responder/<?=$this->dados['slug']?>/resposta/<?=(isset($this->indice) && $this->indice ? $this->indice.'/' : '')?>' style='margin-top:20px;display:block;'>
