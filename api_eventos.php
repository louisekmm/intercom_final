...
$eventos = $dsn->query('select ee.*, date_format(ee.data, "%d/%m/%Y") as data, date_format(ee.hora, "%H:%i") as hora, ea.idEntidade_agrupamento from entidade_evento ee INNER JOIN entidade_evento_agrupamento ea ON ea.idEntidade_evento=ee.id WHERE DATE_SUB(data, INTERVAL 1 DAY)=CURRENT_DATE or DATE_SUB(data, INTERVAL 7 DAY)=CURRENT_DATE')->fetchAll();
...
$usuarios = $dsn->prepare('SELECT  ep.nome as pessoa, ep.celular,  ea.nome as agrupamento, e.nome as entidade FROM entidade_pessoa ep INNER JOIN entidade e ON e.id=ep.idEntidade INNER JOIN entidade_agrupamento ea ON ea.id=ep.idEntidade_agrupamento  WHERE ea.id=? ORDER BY ep.nome ASC');
...
foreach($eventos as $obj) {
	...
	if($obj['sms']==1){
		$grupoenvio = $dsn->query('select * from grupo_envio WHERE id=1')->fetch(PDO::FETCH_ASSOC);
	}
	else{
		$grupoenvio = $dsn->query('select * from grupo_envio WHERE id=2')->fetch(PDO::FETCH_ASSOC);
	}
