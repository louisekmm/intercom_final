...
class Entidade_pessoaController {
	...
	const TABLE = PROJECT_PREFIX.'entidade_pessoa';
	...
	public static function _doimport($a, $b) {
		...
		$selEntidade = $di->getDb()->prepare('SELECT id FROM '.PROJECT_PREFIX.'entidade WHERE nome=?');
		$selAgrupamento = $di->getDb()->prepare('SELECT id FROM '.PROJECT_PREFIX.'entidade_agrupamento WHERE nome=? AND idEntidade=?');
		//$selForm = $di->getDb()->prepare('SELECT id FROM formulario WHERE nome=?');
		$selGrupo = $di->getDb()->prepare('SELECT id FROM '.PROJECT_PREFIX.'grupo_envio WHERE nome=?');
		
		$add = $di->getDb()->prepare('INSERT INTO '.PROJECT_PREFIX.'entidade_pessoa (idEntidade, idEntidade_agrupamento, idGrupo_envio, nome, ra, turno, responsavel, celular) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
		...
		$proj = DefaultController::getProjeto();
			header('Location: '.URL.'/'.$proj.'/entidade_pessoa/import/'.$b.'/');
	}
