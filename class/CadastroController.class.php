...
class CadastroController {
	...
	public function _finalizar() {
		...
		try {
			...
			$insertEntidade = $di->getDb()->prepare('INSERT INTO '.PROJECT_PREFIX.'admin_entidade (idAdmin, idEntidade) VALUES (?, ?)');
	...
	public function _turmas() {
		...
		//	$resultado = '';
			$selTipo = $di->getDb()->prepare('SELECT * FROM '.PROJECT_PREFIX.'admin_tipo WHERE id=?');
			...
			if ($selTipo->rowCount()) {
				...
				if (($da['idAcl']) == 5) {
					...
					$selTurmas = $di->getDb()->prepare('SELECT t.*, e.nome as entidade FROM '.PROJECT_PREFIX.'entidade_agrupamento t INNER JOIN '.EntidadeController::TABLE.' e ON e.id=t.idEntidade WHERE t.idEntidade IN ('.$qMarks.') AND t.idAdmin IS NULL');
