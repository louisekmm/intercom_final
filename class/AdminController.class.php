...
class AdminController {
	...
	function _alocar() {
		...
		if (LoginController::getTipo($di) != 1) {
			$idAdm = LoginController::getUserId($di);
			$selTipo = $di->getDb()->prepare('SELECT ea.id, ea.nome, ea.idAdmin FROM entidade_agrupamento ea INNER JOIN admin_entidade ON ae.idEntidade=ea.idEntidade WHERE ae.idAdmin='.$idAdm);
			$selTipo->Execute();
		...
		$results = $di->getDb()->prepare('SELECT count(*) from entidade_agrupamento WHERE 1=1 AND ('.implode(' OR ', $whereor).')');
		...
		$sel = $di->getDb()->prepare('SELECT ea.id, ea.nome as grupo, idAdmin FROM entidade_agrupamento ea WHERE 1=1 AND ('.implode(' OR ', $whereor).') ORDER BY ea.nome ASC LIMIT '.($pagina-1)*RESULTADOSPAGINA.','.RESULTADOSPAGINA);
		$sel->Execute($valores);
		$dados = $sel->fetchAll();
		
		//$sel1 = $di->getDb()->prepare('SELECT ea.id, a.nome as respo FROM entidade_agrupamento  ea INNER JOIN admin a ON a.id=ea.idAdmin');
		$sel1 = $di->getDb()->prepare('SELECT ea.id, a.nome as respo FROM entidade_agrupamento  ea INNER JOIN admin a ON a.id=ea.idAdmin');
		$sel1->Execute();
		...
		$di->getView()->load('alocar');

	}

	public function _doalocar() {
		global $di;
		$args = func_get_args();
		//print_r($args);
		//$def = new DefaultController();
		//call_user_func_array(array($def, '_list'), $args);
		//$def->_list($args);
		$sel = $di->getDb()->prepare('SELECT id, nome as grupo, idAdmin, idEntidade FROM entidade_agrupamento  WHERE id='.$args[1]);
		$sel->Execute();
		$dados = $sel->fetchAll();
		$esc = ($dados[0]['idEntidade']);
		$sel1 = $di->getDb()->prepare('SELECT a.id, a.nome as respo FROM admin_entidade ae INNER JOIN admin a ON a.id=ae.idAdmin WHERE ae.id='.$esc);
		$sel1->Execute();
		$dados1 = $sel1->fetchAll();
		...
		$di->getView()->results1 = $dados1;
		$di->getView()->lng = LoginController::getLng($di);
		$di->getView()->load('aloca');
		
		
	}
	public function _savealocar() {
		global $di;
		$v = $_POST['responsavel'];
		$a = $_POST['agrupamento'];
		echo ($v."-".$a);
 		$atualizar = $di->getDb()->prepare("UPDATE entidade_agrupamento set idAdmin=".$v." WHERE id=".$a);
		$atualizar->Execute();
		$palavra = Meta::getLangFile('operacao-admin', $di);
		$di->getSession()->setMessage($palavra, true);
		$proj = DefaultController::getProjeto();
		header('Location: '.URL.'/'.$proj.'/admin/alocar/');
	...
	public static function _doimport() {
		...
		$proj = DefaultController::getProjeto();
			header('Location: '.URL.'/'.$proj.'/admin/import/');
	}
...
//		return array(1=> array('name'=>'Lista de presença', 'btn-class'=>'btn-warning', 'class'=>'', 'icon'=> 'th-list', 'href' => 'turma_presenca/list/1/'));
	...
	public static function getFormularios($di) {
		...
		$sel = $di->getDb()->prepare('SELECT * from '.FormularioController::TABLE.' f INNER JOIN '.PROJECT_PREFIX.'cadastrador_formulario fc ON fc.idFormulario=f.id WHERE fc.idAdmin=?');
