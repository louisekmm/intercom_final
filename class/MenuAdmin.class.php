...
class MenuAdmin  {
	...
	const TABLE = PROJECT_PREFIX.'menu_admin';
	
	public static function getMenu($di) {
		$db= $di->getDb();
		$r = $db->query('SELECT t.*,i.valor as icone from '.self::TABLE.' t LEFT JOIN '.PROJECT_PREFIX.'icone i ON i.id=t.idIcone ORDER BY top DESC, nome ASC, controller ASC')->fetchAll(PDO::FETCH_ASSOC);
