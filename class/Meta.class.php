...
class Meta {
	use ParseOptions;
	const TABLE = PROJECT_PREFIX.'meta';
	...
	public static function getTableContents($di, $table) {
		...
		$q = $db->prepare('SELECT id, nome FROM '.(self::getTableName($table) != 'admin' ? PROJECT_PREFIX : '').self::getTableName($table).' ORDER BY '.(self::getTableName($table) == 'mes' ? 'id' : 'nome').' ASC');
		$q->Execute(array());
		...
		if ($foreign2) {
			...
			for($x=count($foreign2)-1;$x>=0;$x=$x-2) {
				if ($x==count($foreign2)-1) {
					$inner[] = 'INNER JOIN '.PROJECT_PREFIX.$foreign2[$x-1].' t'.$x.' ON   t'.$x.'.'.end($foreign2).'='.$valor1;
				
				} else {
					$inner[] = 'LEFT JOIN '.PROJECT_PREFIX.$foreign2[$x-1].' t'.$x.' ON t'.$x.'.'.$foreign2[$x+2].'=t'.($x+2).'.'.$foreign2[$x+2];
				}
			}

			$order = ($table == 'mes' ? 't.id' : 't.nome');
			$result = $db->query('SELECT t.* FROM '.PROJECT_PREFIX.$table.' t '.implode(' ', $inner).'  ORDER BY '.$order.' ASC')->fetchAll(PDO::FETCH_ASSOC);
			...
			try {
				...
				$sel = $db->prepare('SELECT * FROM '.($table != 'admin' ? PROJECT_PREFIX: '').$table.' WHERE '.implode(' AND ', $where).' ORDER BY '.$order.' ASC');
	...
	public static function gInput($info, $args = array(), $args2 = array(), $prepend = '', $div = false, $x=0, $tabs = array(), $permissions) {
		...
		if ($comment) {
			...
			if ($matches) {
				...
				if($label == "Professor Responsavel"){
			...
			if (in_array('bool', $comment)) {
				...
				$arr = array('1'=> $palavra , 0=> $palavra1);
			}
	...
	public static function prepare($di, $info, $table, $args) {
		...
		foreach($info as $item) {
			...
			if (self::isColumn($comment)) {
				...
				if($title == "Professor_Responsavel"){
	...
	public static function getTabelaGenerica($di, $tabela, $id = 0, $key='id') {
		...
		if ($id) {
			...
			$r = $di->getDb()->query("SELECT t.* from ".($tabela != 'admin' ? PROJECT_PREFIX : '').$tabela." t where t.".$key."='".$id."'");
	...
	public static function metaColumns($di, $tabela) {
		$q = $di->getDb()->prepare("SHOW FULL COLUMNS FROM ".($tabela != 'admin' ? PROJECT_PREFIX.$tabela : $tabela));
