...
class LoginController {
	...
	public static function checkLogged() {
		...
		if (!self::isLogged($di)) {
			header('Location: '.URL.ADMIN_FOLDER.'/'.$proj.'/login/');
			die;
		}
	}
	...
	public static function login($di, $userId, $login, $tipo, $super, $table, $nome, $foreign) {
		...
		$di->getSession()->setSessionValue('log', 'logado');
	...
	public static function logout($di) {
		...
		$di->getSession()->unsetSessionValue('log');
	}
	...
	function _doesqueci() {
		...
		$email = $di->getDb()->prepare('SELECT id, email, nome FROM '.self::TABLE.' WHERE email=?');
			...
			if (!$dados) {
				...
				$url = URL.ADMIN_FOLDER.'/'.$proj.'/login/recuperar/'.$token.'/';
