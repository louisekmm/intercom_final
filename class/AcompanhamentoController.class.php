...
class AcompanhamentoController {
	...
	public function _load_avaliadores() {
		...
		$re = $di->getDb()->prepare('SELECT a.nome, at.nome as agrupamento FROM '.PROJECT_PREFIX.'entidade_agrupamento at  LEFT JOIN admin a ON a.id=at.idAdmin WHERE at.idEntidade=?');
	...
	public static function getLogins($di) {
		...
		$selEntidades = $db->prepare('SELECT * FROM '.PROJECT_PREFIX.'entidade WHERE 1=1 AND ('.implode(' OR ', $whereor).')');
		//echo 'SELECT * FROM '.PROJECT_PREFIX.'entidade WHERE 1=1 AND ('.implode(' OR ', $whereor).')';
		$selAvaliadores = $db->prepare('SELECT count(*) FROM '.PROJECT_PREFIX.'admin_entidade ae INNER JOIN admin a ON a.id=ae.idAdmin INNER JOIN '.PROJECT_PREFIX.'admin_tipo at ON at.id=a.idAdmin_Tipo AND at.idAcl=5 WHERE ae.idEntidade=?');
		$selDiretores = $db->prepare('SELECT count(*) FROM '.PROJECT_PREFIX.'admin_entidade ae INNER JOIN admin a ON a.id=ae.idAdmin INNER JOIN '.PROJECT_PREFIX.'admin_tipo at ON at.id=a.idAdmin_Tipo AND at.idAcl=3 WHERE ae.idEntidade=?');
		$selTurmas = $db->prepare('SELECT count(*) FROM '.PROJECT_PREFIX.'entidade_agrupamento  WHERE idEntidade=?');
	...
	public static function getPreenchimentos($di) {
		...
		$selEntidades = $db->prepare('SELECT e.id, e.nome as entidade, a.nome as avaliador, ea.nome as agrupamento FROM '.PROJECT_PREFIX.'entidade e INNER JOIN '.PROJECT_PREFIX.'entidade_agrupamento ea ON ea.idEntidade=e.id INNER JOIN admin a ON a.id=ea.idAdmin WHERE 1=1 AND ('.implode(' OR ', $whereor).') ORDER BY e.nome, ea.nome');
		//$selSemanas = $db->prepare('SELECT * FROM '.PROJECT_PREFIX.'semana ORDER BY termino DESC');
