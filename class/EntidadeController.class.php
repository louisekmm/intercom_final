...
class EntidadeController {
	...
	const TABLE = PROJECT_PREFIX.'entidade';
	const NAME = 'Escola';
	...
	public function _minhas() {
		...
		if (LoginController::getTipo($di) != 1) {
			$selTipo = $di->getDb()->prepare('SELECT idEntidade FROM '.PROJECT_PREFIX.'admin_entidade WHERE idAdmin=?');
	...
	public function _agrupamentos() {
		...
		if (LoginController::getTipo($di) != 1) {
			$selTipo = $di->getDb()->prepare('SELECT idEntidade, idAdmin_Tipo FROM '.PROJECT_PREFIX.'admin_entidade WHERE idEntidade=? AND idAdmin=?');
			...
			/*$selTipo = $di->getDb()->prepare('SELECT idEntidade, idAdmin_Tipo FROM '.PROJECT_PREFIX.'admin_entidade WHERE idEntidade=?');
			$selTipo->Execute(array($args[0]));
		...
		$di->getView()->semanas = $di->getDb()->query('SELECT s.*, d.nome as dimensao FROM '.PROJECT_PREFIX.'semana s INNER JOIN '.PROJECT_PREFIX.'dimensao d ON d.id=s.idDimensao WHERE s.inicio<=CURRENT_DATE AND s.termino>=CURRENT_DATE ORDER BY termino DESC')->fetchAll();
	...
	public function _avalia() {
		...
		if ($values['semana'] && $values['id']) {
			...
			/*if (LoginController::getTipo($di) != 1) {
				$selTipo = $di->getDb()->prepare('SELECT idEntidade, idAdmin_Tipo FROM '.PROJECT_PREFIX.'admin_entidade WHERE idEntidade=? AND idAdmin=?');
				$selTipo->Execute(array($args[0], LoginController::getUserId($di)));
			...
			$semana = $di->getDb()->prepare('SELECT s.*, d.nome as dimensao, d.id as idDimensao, DATE_FORMAT(inicio, "%d/%m/%Y") as inicio, DATE_FORMAT(termino, "%d/%m/%Y") as termino, IF(termino < current_date(), 1, 0) as expirado  FROM '.PROJECT_PREFIX.'semana s INNER JOIN '.PROJECT_PREFIX.'dimensao d ON d.id=s.idDimensao WHERE s.id=?');
			$semana->Execute(array($values['semana']));
			$di->getView()->semana = $semana->fetch();
			
			
			
			$notas = $di->getDb()->prepare('SELECT valor, descricao FROM '.PROJECT_PREFIX.'dimensao_descricao WHERE idDimensao=? ORDER BY valor ASC');
			$notas->Execute(array($di->getView()->semana['idDimensao']));
			$di->getView()->notas = $notas->fetchAll();
			
			//$di->getView()->semanas = $di->getDb()->query('SELECT s.*, d.nome as dimensao FROM '.PROJECT_PREFIX.'semana s INNER JOIN '.PROJECT_PREFIX.'dimensao d ON d.id=s.idDimensao ORDER BY termino DESC')->fetchAll();
			
			$sel = $di->getDb()->prepare('SELECT ema.idSemana, em.*, em.id as idPessoa, ema.nota, ema.nao_matriculado FROM '.self::TABLE.'_pessoa em LEFT JOIN '.self::TABLE.'_pessoa_avaliacao ema ON ema.idEntidade_pessoa=em.id AND ema.idSemana=? WHERE em.idEntidade_agrupamento=?');
			$sel->Execute($valores);
			$dados = $sel->fetchAll();

			$sel1 = $di->getDb()->prepare('SELECT dd.id, dd.valor, dd.descricao FROM dimensao_descricao dd INNER JOIN semana s ON s.idDimensao=dd.idDimensao WHERE s.id='.$dados[0]['idSemana']);
			$sel1->Execute();
			$dados1 = $sel1->fetchAll();
			
		
			$di->getView()->results1 = $dados1;
			$di->getView()->results = $dados;
			$di->getView()->lng = LoginController::getLng($di);
	...
	public function _salva_avaliacao() {
		...
		if ($values['semana'] && $values['turma']) {
			...
			header("Location: ".URL.ADMIN_FOLDER.'/'.$proj.'/entidade/minhas');
		}
