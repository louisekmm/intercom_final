...
class FormularioController {
	...
	const TABLE = PROJECT_PREFIX.'formulario';
...
/*		if (LoginController::getTipo($di) == 2) {
	...
	public static function _exportar($a='', $slug_formulario = '') {
		...
		$selPerguntas = $di->getDb()->prepare('
					...
					FROM '.PROJECT_PREFIX.'resposta_otimizada r
					INNER JOIN '.PROJECT_PREFIX.'formulario f ON f.id=r.idFormulario
					INNER JOIN '.PROJECT_PREFIX.'cadastrador_formulario fc ON fc.idFormulario=f.id
	...
	public static function _doimport($a, $b) {
		...
		$selPergunta = $di->getDb()->prepare('SELECT id FROM '.PROJECT_PREFIX.'formulario_pergunta WHERE idFormulario=:formId ORDER BY ordem ASC limit 1 OFFSET :page ');
		$selMax = $di->getDb()->prepare('SELECT max(indice) FROM '.PROJECT_PREFIX.'resposta WHERE idPergunta=? AND idAdmin=?');
		$selMax = $di->getDb()->prepare('SELECT max(indice) FROM '.PROJECT_PREFIX.'resposta r INNER JOIN formulario_pergunta f ON f.id=r.idPergunta WHERE idFormulario=? AND r.idAdmin=? ORDER BY r.id DESC limit 1');
		$addResposta = $di->getDb()->prepare('INSERT INTO '.PROJECT_PREFIX.'resposta (idPergunta, resposta, idAdmin, indice, pre) VALUES (?, ?, ?, ?, ?)');
		...
		$proj = DefaultController::getProjeto();
			header('Location: '.URL.'/'.$proj.'/formulario/import/'.$idForm.'/');
	}
	...
	public static function _duplicar($a='', $id = '') {
		...
		$proj = DefaultController::getProjeto();
		header('Location: '.URL.'/'.$proj.'/formulario/list/');
	...
	public static function _responder($arg1 = '', $slug_formulario = '', $perguntas = '', $indice = 0) {
		...
		$di->getView()->lng = LoginController::getLng($di);
		if ($slug_formulario) {
			$sel = $di->getDb()->prepare('SELECT t.*, fc.pre, fc.respondido, fc.numero, m.file as arquivo1, m2.file as arquivo2 FROM '.self::TABLE.' t LEFT JOIN '.PROJECT_PREFIX.'midia m ON m.id=t.idMidia LEFT JOIN '.PROJECT_PREFIX.'midia m2 ON m2.id=t.idMidia2 INNER JOIN '.PROJECT_PREFIX.'cadastrador_'.self::TABLE.' fc ON fc.idFormulario=t.id WHERE t.slug=? AND fc.idAdmin=?');
			...
			if ($dados) {
				...
				if ($perguntas == 'perguntas') {
					...
					if ($dados['respondido'] >= $dados['numero'] && !$indice) {
						...
						if ($dados['pre']) {
							$selPerguntas = $di->getDb()->prepare('SELECT fp.*, r.resposta, r.indice, f.slug, r.pre as pre2 FROM '.self::TABLE.'_pergunta fp LEFT JOIN '.PROJECT_PREFIX.'resposta r ON r.idPergunta=fp.id AND r.idAdmin=? INNER JOIN '.self::TABLE.' f ON f.id=fp.idFormulario WHERE fp.idFormulario=? AND fp.ordem=1 ORDER BY r.indice ASC');
						...
						if ($indice) {
							...
							$selPerguntas = $di->getDb()->prepare('SELECT fp.*, r.resposta FROM '.self::TABLE.'_pergunta fp LEFT JOIN '.PROJECT_PREFIX.'resposta r ON r.idPergunta=fp.id AND r.indice=? AND r.idAdmin=?  WHERE fp.idFormulario=? ORDER BY fp.ordem ASC');
					...
					if (($dados['respondido'] >= $dados['numero'] || $_POST['indice'] != $dados['respondido']) && !$indice) {
						header("Location: ".URL.'/'.$proj.'/formulario/responder/'.$dados['slug'].'/perguntas/');
						exit();
					} else {
						...
						if ($indice) {
							$updateResposta = $di->getDb()->prepare('UPDATE '.PROJECT_PREFIX.'resposta SET resposta=? WHERE idPergunta=? AND  idAdmin=? AND indice=?');
							//$updateResposta2 = $di->getDb()->prepare('UPDATE resposta_otimizada SET resposta=? WHERE idFormulario=? AND  idAdmin=? AND indice=?');
							$selResposta = $di->getDb()->prepare('SELECT id, pre FROM '.PROJECT_PREFIX.'resposta WHERE idPergunta=? AND  idAdmin=? AND indice=?');
						}
						
						$insertResposta = $di->getDb()->prepare('INSERT INTO '.PROJECT_PREFIX.'resposta (idPergunta, resposta, idAdmin, indice) VALUES (?, ?, ?, ?)');
						$insertResposta2 = $di->getDb()->prepare('INSERT INTO '.PROJECT_PREFIX.'resposta_otimizada (idFormulario, resposta, idAdmin, indice, r1) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY update resposta=VALUES(resposta), r1=VALUES(r1)');
						$updateCadastrador = $di->getDb()->prepare('UPDATE '.PROJECT_PREFIX.'cadastrador_'.self::TABLE.' SET respondido=respondido+1 WHERE idFormulario=? and idAdmin=?');
						...
						try {
							...
							foreach($valores as $i=>$resp) {
								...
								if ($indice) {
									...
									if (isset($temp['pre']) && $temp['pre']) {
										$pre = true;
										$up = $di->getDb()->prepare('UPDATE '.PROJECT_PREFIX.'resposta SET pre=0 WHERE id=?');
							...
							if ($dados['pre']) {
								header("Location: ".URL.'/'.$proj.'/formulario/responder/'.$dados['slug'].'/completar/'.$indice.'/');
							} else {
								if ($indice) {
									header("Location: ".URL.'/'.$proj.'/formulario/responder/'.$dados['slug'].'/perguntas/'.$indice.'/');
								} else {
									header("Location: ".URL.'/'.$proj.'/formulario/responder/'.$dados['slug'].'/perguntas/');
								}
							}
				...
				header('Location: '.URL.ADMIN_FOLDER.'/'.$proj.'/index/');
			}
		} else {
			header('Location: '.URL.ADMIN_FOLDER.'/'.$proj.'/index/');
		}
