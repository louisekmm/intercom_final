...
class DefaultController {
	...
	public static function getProjeto(){
		global $di;
		 $resultsdb = $di->getDb()->prepare("SELECT DATABASE()");
        $resultsdb->Execute();
        $proj = $resultsdb->fetch();
        $projeto = $proj['DATABASE()'];
        return ($projeto);
	}

	public static function getBot($di){
			$db = $di->getDb();
			$results = $db->prepare("SELECT count(*) from config WHERE nome='Bot'");
			$results->Execute();
			$total = $results->fetch();
			
			if($total['count(*)'] > 0){
				$resultsdb = $db->prepare("SELECT DATABASE()");
				$resultsdb->Execute();
				$proj = $resultsdb->fetch();
				return ($proj);
			}
...
function _direta() {
			global $di;
			LoginController::checkLogged();
			$usuarios = $di->getDb()->query('SELECT idEntidade_agrupamento FROM entidade_pessoa GROUP BY idEntidade_agrupamento')->fetchAll();
			$di->getView()->grupo = $usuarios;
			$di->getView()->lng = LoginController::getLng($di);
			$di->getView()->load('direta');
}
...
function _enviarmsg() {
		...
		$args = func_get_args();
		$destinatarios = array();
		$destinatarios = $_POST['idEntidade_agrupamento'];
		$msg = $_POST['msg'];
		
		//print_r($destinatarios);
		$usuarios = $di->getDb()->prepare('SELECT nome as pessoa, celular FROM entidade_pessoa WHERE idEntidade_agrupamento=? ORDER BY nome ASC');

		$usuarios->Execute($destinatarios);
		...
		foreach($dados as $obj) {
			...
			$date->add($week);
			$contacts[$conf['api_data_'.PROJECT_NAME]][] = $date->format('Y-m-d H:i:s');
			$contacts[$conf['api_mensagem_'.PROJECT_NAME]][] = $msg;
	...
	$proj = DefaultController::getProjeto();
	header('Location: '.URL.'/'.$proj.'/admin/direta/');
}
	...
	function _criando() {
			...
			if($valor != ''){
				...
				foreach($dados as $d){
					if(($d['Database']) == $valor){
						$palavra = Meta::getLangFile('ja-default', $di);
						$di->getSession()->setMessage($palavra, false);
						$proj = DefaultController::getProjeto();
						header('Location: '.URL.'/'.$proj.'/admin/listar');
						die;
						exit();
					}
				}
				...
				DB::novoProjeto($valor, $di);

				$palavra = Meta::getLangFile('operacao-admin', $di);
				$di->getSession()->setMessage($palavra, true);
			}
			else{
				$palavra = Meta::getLangFile('erro3-default', $di);
				$di->getSession()->setMessage($palavra, false);
			}
			$proj = DefaultController::getProjeto();
			header('Location: '.URL.'/'.$proj.'/admin/listar');
	...
	function _listar() {
			...
			//	print_r($dados);
			$di->getView()->totalResults = $cont;
			$di->getView()->results = $dados;
			$di->getView()->lng = LoginController::getLng($di);
			$di->getView()->load('projeto');
...
//						$inner .= ' INNER JOIN admin a2 ON a2.id=a.idAdmin '; //achou o coordenador
					...
					$selPerguntas = $di->getDb()->prepare('
								...
								FROM '.PROJECT_PREFIX.'formulario t
								INNER JOIN '.PROJECT_PREFIX.'cadastrador_formulario fc ON fc.idFormulario=t.id
	...
	function _doupload() {
		...
		$proj = DefaultController::getProjeto();
		header('Location: '.URL.'/'.$proj.'admin/imagem/upload/');
	}
	...
	function _list() {
		...
		if ($info) {
			...
			foreach($all['columns'] as $column) {
				...
				if ($column['midia']) {
					...
					if ($column['foreign']) {
						...
						$inner[] = 'LEFT JOIN '.(Meta::getTableName($nomeTemp) != 'admin' ? PROJECT_PREFIX : '').Meta::getTableName($nomeTemp).' '.$temp.' ON '.$temp.'.id=t.'.$column['name'];
			...
			//echo ($all['controller'] != 'admin' ? PROJECT_PREFIX: '').$all['controller'].'-';
			//print_r($inner);
			$results = $db->prepare("SELECT count(*) from ".($all['controller'] != 'admin' ? PROJECT_PREFIX: '').$all['controller']." t ".implode(' ', $inner).' WHERE '.implode(' AND ', $where));
			...
			$results = $db->prepare("SELECT ".implode(',', $columns)." from ".($all['controller'] != 'admin' ? PROJECT_PREFIX : '').$all['controller']." t ".implode(' ', $inner)." WHERE ".implode(' AND ', $where)." ORDER BY ".$all['order'].' '.$all['orderby'].'  LIMIT '.($pagina-1)*RESULTADOSPAGINA.','.RESULTADOSPAGINA);
	...
	function _order() {
		...
		try {
			...
			if ($infoTable) {
				...
				$r = $db->prepare('UPDATE '.PROJECT_PREFIX.$args[0].' SET ordem=? WHERE id=?');
	...
	function _enviar() {
		...
		if ($infoTable) {
			...
			if($r1['tipo'] == 'Evento'){
				...
				header('Location: '.URL.ADMIN_FOLDER.'/'.$proj.'/grupo_envio/list');
				die;
				exit();
	...
	public static function notAllowed($di) {
		...
		header("Location: ".URL.ADMIN_FOLDER.'/'.$proj);
		exit();
	}
		...
		try {
			if ($args[0] && $args[1]) {
				...
				if($args[0] == 'grupo_envio' && ($args[1] == 1  || $args[1] == 2 )){
					...
					$r = $db->prepare("DELETE from ".($args[0] != 'admin' ? PROJECT_PREFIX: '').$args[0].' WHERE id=?');
		...
		try {
			...
			if ($infoTable) {
				...
				if (!$values['id'] && in_array('order', $options)) {
					...
					$r = $di->getDb()->query("SELECT MAX(ordem) from ".($args[0] != 'admin' ? PROJECT_PREFIX: '').$args[0].$where);
				...
				$result = $di->getDb()->replace(($args[0] != 'admin' ? PROJECT_PREFIX : '').$args[0], $values);
					...
					if (in_array('1n', $field['comment']) && !in_array('disabled', $field['comment'])) {
						...
						if (count($fields)) {
							...
							foreach($primeiro as $x) {
								...
								$di->getDb()->replace(PROJECT_PREFIX.$field['name'], $arrDados);
						...
						$clear = $di->getDb()->prepare('DELETE FROM '.PROJECT_PREFIX.$field['name'].' WHERE id NOT IN ('.$placeHolders.') AND '.$idForeign.'=?');
				...
				if ($ajax) {
					...
					if ($selfonly) {
						header("Location: ".URL.ADMIN_FOLDER.'/'.$proj.'/'.$args[0].'/add/'.$values['id']);
					} else if ($outros['back']) {
						header("Location: ".URL.ADMIN_FOLDER.'/'.$proj.'/'.$args[0].'/add/0/'.$args_text.'?back=1');
					} else {
						header("Location: ".URL.ADMIN_FOLDER.'/'.$proj.'/'.$args[0].'/list/1/'.$args_text);
					}
				}
			...
			if ($ajax) {
				...
				if ($selfonly) {
					...
					header("Location: ".URL.ADMIN_FOLDER.'/'.$proj.'/'.$args[0].'/add/');
				}
			}
		}
	}
